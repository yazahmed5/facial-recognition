...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            button1 = tk.Button(self, text="   Add missing person's dataset ", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("PageOne"))
            button2 = tk.Button(self, text="   search for added faces  ", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("PageTwo"))
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.uploadbutton = tk.Button(self, text="upload Data Set", fg="#ffffff", bg="#263942", command=self.uploadimg)
        ...
        self.uploadbutton.grid(row=1, column=1, ipadx=5, ipady=4, padx=10, pady=20)
        self.trainbutton.grid(row=1, column=2, ipadx=5, ipady=4, padx=10, pady=20)

    def capimg(self):
        self.numimglabel.config(text=str("Captured Images = 0 "))
        messagebox.showinfo("INSTRUCTIONS", "We will Capture 300 pic of your Face.")
        x = start_capture(self.controller.active_name, mode='camimg')
        self.controller.num_of_images = x
        self.numimglabel.config(text=str("Number of images captured = "+str(x)))

    def uploadimg(self):
        x = start_capture(self.controller.active_name, mode='uplimg')
        # Add selected image paths to controller's list
        self.controller.num_of_images = x
        self.numimglabel.config(text="Number of images uploaded = "+str(x))

    def trainmodel(self):
        if self.controller.num_of_images < 30:
            messagebox.showerror("ERROR", "No enough Data, Capture at least 30 images!")
            return
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button4.grid(row=2,column=2, sticky="ew", ipadx=5, ipady=4, padx=10, pady=10)
